...
import { IS_PREVIEW_MODE, IS_PROD_MODE, logDev } from '#src/utils/common';
...
export default class SettingsService {
  ...
  initialize = async () => {
    const configFilePath = IS_PROD_MODE || IS_PREVIEW_MODE ? '/webapp.ini' : '/.webapp.ini';
    ...
    if (!settings) {
      throw new AppError(`Unable to load  ${configFilePath}`, errorPayload);
    }
